...
#  En este archivo se ejecuta la simulación donde se realizan los saltos de tiempo por eventos.

def main():
    pantallaInicial()   # Carga y preparacion
    ...
    tabla_particiones: List[Particion] = [              # Definición de Particiones Fijas
    ...
    while procesos_terminados_count < procesos_totales_count:       # BUCLE PRINCIPAL DE SIMULACIÓN. Se ejecuta mientras haya procesos por finalizar
        eventos_T = [] # Aca se listan los eventos ocurridos en este instante T
        ...
        # tiene mayor prioridad (menos TI) que alguien en RAM para hacer un swap.
        ...
        proximo_arribo_t = float('inf')             # A. ¿Cuándo es el próximo ARRIBO?
        ...
        fin_cpu_t = float('inf')                    # B. ¿Cuándo termina el proceso actual en CPU?
        id_proceso_cpu = None
        if not cpu.esta_libre():
            fin_cpu_t = T + cpu.tiempo_restante_irrupcion
            id_proceso_cpu = cpu.proceso_en_ejecucion.idProceso

        proximo_evento_t = float('inf')             # C. Determinar el evento más cercano (Ganador  )
        razon_salto = ""
        icono_evento = ""

        if proximo_arribo_t < fin_cpu_t:            # Distintos eventos que pueden ocurrir
        ...
        if proximo_evento_t == float('inf'):            # D. Calcular la magnitud del salto
        ...
        if not eventos_T:       # Impresión de Logs de Eventos
        ...
        if procesos_terminados_count >= procesos_totales_count:     # Comprobamos la condición de salida
            console.print("\n[bold green]\n\n Finalizaron todos los procesos :). [/bold green]")
            input("Presione 'Enter' para realizar el informe estadístico.")
            limpiar_pantalla()
            break

        try:    # Espera de alguna entrada del usuario para avanzar al proximo paso
            mensaje_input = (
    ...
    mostrar_informe_estadistico(cola_terminados, T)         # Generación del informe estadístico con los procesos finalizados
